...
//       winAh.realFetch.call(win, request.url, init).then(async (res) => {
//         console.log("res", res);
//         if (typeof request.response === "function") {
...
//                 const modifiedResponse = request.response(response) || response;
                
//                 // 创建新的Response对象返回
//                 const mockResponse = new Response(
//                   modifiedResponse.text || modifiedResponse.responseText || JSON.stringify(modifiedResponse.json),
//                   {
//                     status: modifiedResponse.status || 200,
//                     statusText: modifiedResponse.statusText || "OK",
//                     headers: new Headers(modifiedResponse.responseHeaders || {})
//                   }
//                 );
...
//                     if (key === 'json') {
//                       return Promise.resolve(modifiedResponse.json || JSON.parse(modifiedResponse.text || '{}'));
//                     } else if (key === 'text') {
//                       return Promise.resolve(modifiedResponse.text || JSON.stringify(modifiedResponse.json || ''));
//                     } else {
...
//           } catch (e) { console.error("response 回调内部又抛错:", e) }
//         }
